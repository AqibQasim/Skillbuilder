...
async function createOrderService(userId, items) {
    const orderCreate = { user_id: userId, status: 'pending', total_amount: 0, currency: 'usd' };
    ...
    for (const item of items) {
        const course = await courseRepository.findOneById(item.courseId); // Using findOne to find a course by ID
        if (!course) {
...
async function initiateCheckoutService(userId, items) {
    ...
    const courses = await Promise.all(items.map(async (item) => {
...
module.exports = { createOrderService, getOrderService, initiateCheckoutService };
